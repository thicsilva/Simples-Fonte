...
uses
  ...
  Vcl.Buttons, Vcl.Imaging.pngimage, BancoService;

type
  TFBanco = class(TFModelo)
    fdmt_ModeloDESCRICAO: TStringField;
    fdmt_ModeloBANCO: TStringField;
    fdmt_ModeloTIPO_CONTA: TStringField;
    fdmt_ModeloCONFIG_BOLETO: TStringField;
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
    Label4: TLabel;
...
uses BancoStore, Mensagem, Menu;

procedure TFBanco.ActionAlrerarExecute(Sender: TObject);
begin
  if  not fdmt_Modelo.IsEmpty then
  begin
    varBaseIdBanco:= fdmt_ModeloID.AsString;
    OpenForm(FBancoStore,TFBancoStore);
    fdmt_Modelo.Open;
    TBancoService.index(fdmt_Modelo,'');
  end;
end;

procedure TFBanco.ActionCadastrarExecute(Sender: TObject);
begin
  varBaseIdBanco:= EmptyStr;
  OpenForm(FBancoStore,TFBancoStore);
  fdmt_Modelo.Open;
  TBancoService.index(fdmt_Modelo,'');
end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            if TBancoService.destroyer(fdmt_ModeloID.AsString) then
              fdmt_Modelo.Delete;
          end;
...
procedure TFBanco.edtPesquisaChange(Sender: TObject);
begin
  if Trim(edtPesquisa.Text) <> EmptyStr then
    TBancoService.index(fdmt_Modelo,Trim(edtPesquisa.Text));
end;
