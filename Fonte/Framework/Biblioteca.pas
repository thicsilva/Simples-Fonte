...
uses
  ...
  Inifiles, DBClient, DB, VO ,SqlExpr, DBXMySql, Grids, DBGrids,
  IdHashMessageDigest, Constantes, Math, Rtti, TypInfo,
  {$IFDEF VER210} SWSystem, {$ELSE} IWSystem, {$ENDIF} DBXJSON, Atributos, StrUtils,
  System.JSON, Vcl.StdCtrls, Bibli;
...
function RemoveTracoENumeros(Texto: String): String;
...
procedure CtrlResize(var Sender: TForm);
//Aqui vc especifica a resoluÁ„o que vc desenvolveu o aplicativo
const
iWidth = 1024;
iHeight = 768;
...
begin
  ...
  if Components[i] is TWinControl then
    begin { Redefine os componentes em proporÁ„o ao original }
...
function DevolveConteudoDelimitado(Delimidador:string;
...
begin
  ...
  // verifica a existÍncia do Ìndice e propriedades
  ...
  begin
    ...
    begin
      ...
      // determina como deve ser criado o Ìndice, inverte a condiÁ„o ixDescending
  ...
  // caso n„o encontre o Ìndice ou o mesmo esteja em uso
  ...
  begin
    ...
    try
      ...
      // se Ìndice indeterminado, seta o padr„o
      if bolUsed then
        strColumn := idxDefault;
    end;
  end;
  ...
  // tenta setar o Ìndice, caso ocorra algum erro seta o padr„o
  try
...
//  begin
//    Campo := pObj.Pairs[i].JsonString.Value;
//    if Campo = 'fields' then
//    begin
//      jObj := pObj.Pairs[i].JsonValue as TJSONObject;
//      for F := 0 to jObj.Count - 1 do
//      begin
//        Campo := jObj.Pairs[F].JsonString.Value;
//        if LowerCase(Campo) = LowerCase('F' + pCampo) then
//        begin
//          Result := jObj.Pairs[F].JsonValue.ToString;
//          Break;
//        end;
//      end;
//    end;
//  end;
  ...
  // Remove ˙ltima aspa se ela existir
  if Length(Result) > 0 then
  begin
...
// funÁ„o auxiliar para converte UF do cliente para codigo
function UFToInt(pUF: String): integer;
begin
  Result := 0;
  if pUF = 'RO' then
...
// funÁ„o auxiliar para converte Codigo UF do cliente para UF
function IntToUF(pUF: integer): String;
begin
  Result := '';
  if pUF = 11 then
